...
const TitleScreen: React.FC = () => {
    ...
    const checkForAllValidReinforcements = (id: string, playerid: number) => {
        let validbuttonid = [];
        let buttonqueue = [id];
        ...
        while (buttonqueue.length !== 0) {
            ...
            for (const territory of adjDict.dict[curbut]) {
                const curbutton = buttonData.find(button => button.id === territory);
                if (!visited[territory] && curbutton.playerid === playerid) {
                    buttonqueue.push(territory); // Enqueue unvisited adjacent territories
                    visited[territory] = true; // Mark as visited
                }
            }
        }
