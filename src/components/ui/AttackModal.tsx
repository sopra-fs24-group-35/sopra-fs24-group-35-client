...
const AttackModal = ({isModalOpen, modalContent, onClose, lobbyId, gameId}) => {
  ...
  const attack = async () => {
    ...
    const requestBody = JSON.stringify({
      ...
      "troopsAmount": selectedTroops,
      "repeats": selectedAttacks
    });
  ...
  /*  const attack = async() => {
      const config = { Authorization: localStorage.getItem("lobbyToken") };
      const requestBody = JSON.stringify({"attackingTerritory" : attackTerritory.name,"defendingTerritory" : defenseTerritory.name, "troopsAmount" : selectedTroops,"repeats" : 1});
      console.log("requestBody: ", requestBody);
